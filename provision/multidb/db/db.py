...
class MultiDb(object):
    ...
    def provision_logical_dbs(self, yaml_parser, aws_ssm):
        ...
        try:
            ...
            db_host = aws_ssm.ssm_get_parameter('SPRYKER_DB_HOST')
            db_root_user_name = aws_ssm.ssm_get_parameter('SPRYKER_DB_ROOT_USERNAME')
            db_root_password = aws_ssm.ssm_get_parameter('SPRYKER_DB_ROOT_PASSWORD')
    ...
    def read_database_configuration(self, yaml_parser, aws_ssm):
         ...
         db_host = aws_ssm.ssm_get_parameter('SPRYKER_DB_HOST')
         db_port = aws_ssm.ssm_get_parameter('SPRYKER_DB_PORT')
         ...
         data['databases']['_default'] = {
             ...
             'username': aws_ssm.ssm_get_parameter('SPRYKER_DB_USERNAME')['Parameter']['Value'],
             'password': aws_ssm.ssm_get_parameter('SPRYKER_DB_PASSWORD')['Parameter']['Value'],
             'character-set': 'utf8',
         ...
         for region_name, region_data in deploy_file_data['regions'].items():
            if 'database' in region_data['services']:
                ...
                aws_ssm.ssm_put_parameter('SPRYKER_PAAS_SERVICES', json.dumps(data), aws_ssm.PARAM_STORE_TYP, aws_ssm.PARAM_STORE_SECRET)
                exit(0)
