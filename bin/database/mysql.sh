...
{
    tableCount=$(execSpryker 'VERBOSE=0 MYSQL_PWD="${SPRYKER_DB_ROOT_PASSWORD}" mysql -h ${SPRYKER_DB_HOST} -u ${SPRYKER_DB_ROOT_USERNAME} -e "SELECT TABLE_NAME FROM information_schema.tables WHERE table_schema = \"${SPRYKER_DB_DATABASE}\"" | wc -l | sed "s/^ *//"  | tr -d " \n\r" || echo 0')
