...
function Images::push() {
    ...
    for application in "${SPRYKER_APPLICATIONS[@]}"; do
        ...
        # Continue on fail to keep compatibility with outdated Terraform code bases, when '*-cache' ECR repository doesn't exist
        # With this it will still work, but will take more time to due lack of cache
        docker push "${AWS_ACCOUNT_ID}.dkr.ecr.${AWS_REGION}.amazonaws.com/${SPRYKER_PROJECT_NAME}-cache:composer-cache-latest" | true
}
