...
{
    $environmentConfigurationData = getEnvironmentConfigurationDataFromProjectData($projectData);

    if (empty($environmentConfigurationData)) {
        return [];
    }

    $environmentConfigurationOriginData = getEnvironmentConfigurationDataFromProjectEnvFile($deploymentDir);
    $gitIgnoreData = getGitIgnoreProjectData($deploymentDir);

    $isEnvironmentConfigurationValid = validationEnvironmentConfiguration(
        $environmentConfigurationOriginData,
...
{
    if (!file_exists($deploymentDir . DS . ENV_DOCKER_LOCAL_FILE_NAME)) {
        return [];
    }
    return Dotenv::createImmutable($deploymentDir, ENV_DOCKER_LOCAL_FILE_NAME)->load();
}

/**
 * @param string $deploymentDir
 * @param array $environmentConfigurationData
 * @param \Twig\Environment $twig
 *
 * @throws \Twig\Error\LoaderError
 * @throws \Twig\Error\RuntimeError
 ...
 * @return array
 */
function buildEnvironmentConfigurationFile(
    string $deploymentDir,
    array $environmentConfigurationData,
    array $environmentConfigurationOriginData,
    Environment $twig
): array {
    $resultData = [];

    foreach ($environmentConfigurationData as $environmentVariableName => $environmentVariableValue) {
        $resultData[$environmentVariableName] = array_key_exists($environmentVariableName, $environmentConfigurationOriginData)
            ? $environmentConfigurationOriginData[$environmentVariableName]
            : $environmentVariableValue;
    }

    file_put_contents(
        $deploymentDir . DS . ENV_DOCKER_LOCAL_FILE_NAME,
        $twig->render('env/env.docker.local.twig', array(
            'environmentConfiguration' => $resultData
        ))
    );
...
/**
 * @param string $deploymentDir
 * @param array $gitIgnoreData
 *
 * @return void
 */
function buildGitIgnoreFile(string $deploymentDir, array $gitIgnoreData): void
{
    $gitIgnoreData[] = '# Generated by Docker/SDK';
    $gitIgnoreData[] = ENV_DOCKER_LOCAL_FILE_NAME;
    $gitIgnoreData[] = '';

    file_put_contents(
        $deploymentDir . DS . GIT_IGNORE_FILE_NAME,
        implode(PHP_EOL, $gitIgnoreData)
    );
}
 ...
 * @return array
 */
function getGitIgnoreProjectData(string $deploymentDir): array
{
    $filePath = $deploymentDir . DS . GIT_IGNORE_FILE_NAME;
...
function validationEnvironmentConfiguration(array $environmentConfigurationOriginData, array $environmentConfigurationData, array $gitIgnoreData): bool
{
    ...
    $envDockerLocalHasExtraVars = [];
    $envDockerLocalHasEmptyVars = [];
    $missingEnvVariables = [];

    foreach ($environmentConfigurationOriginData as $envName => $envValue) {
        if (!array_key_exists($envName, $environmentConfigurationData)) {
            $envDockerLocalHasExtraVars[] = $envName;
        }

        if ($envValue == null) {
            $envDockerLocalHasEmptyVars[] = $envName;
        }
    }

    foreach ($environmentConfigurationData as $envName => $envValue) {
        if (!array_key_exists($envName, $environmentConfigurationOriginData)) {
            $missingEnvVariables[] = $envName;
        }
    }

    $countEnvDockerLocalHasExtraVars = count($envDockerLocalHasExtraVars);
    $countEnvDockerLocalHasEmptyVars = count($envDockerLocalHasEmptyVars);
    $countMissingEnvVariables = count($missingEnvVariables);

    if ($countEnvDockerLocalHasExtraVars == 0 && $countEnvDockerLocalHasEmptyVars == 0 && $countMissingEnvVariables == 0) {
        return true;
    }

    warn( $redColorCode . $boldText . PHP_EOL .  'Warning:');
    warn( $redColorCode . $boldText . PHP_EOL .  'Environment configuration validation errors:');

    if ($countEnvDockerLocalHasExtraVars > 0) {
        buildEnvironmentConfigurationErrorsAsList('`' . ENV_DOCKER_LOCAL_FILE_NAME . '` has extra variables:', $envDockerLocalHasExtraVars);
    }

    if ($countEnvDockerLocalHasEmptyVars > 0) {
        buildEnvironmentConfigurationErrorsAsList('`' . ENV_DOCKER_LOCAL_FILE_NAME . '` has empty variables:', $envDockerLocalHasEmptyVars);
    }
...
function buildEnvironmentConfigurationErrorsAsList(string $headerMessage, array $envVariables)
{
    ...
    warn($redColorCode . $headerMessage);
    warn($listSeparator .  implode(PHP_EOL . $listSeparator, $envVariables));
    warn($redColorCode . '***' . PHP_EOL);
}
...
function getEnvironmentConfigurationDataFromProjectData(array $projectData): array
{
    ...
    foreach ($envConfigSecrets as $envConfigSecret) {
        $result[$envConfigSecret[$envConfigNameKey]] = null;
    }

    foreach ($envConfigParams as $envConfigParam) {
        $result[$envConfigParam[$envConfigNameKey]] = $envConfigDefaultKey;
    }
